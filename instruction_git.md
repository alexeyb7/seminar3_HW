...
### Добавление (связывание) удаленных репозиториев

Для добавления удаленного репозитория на локальный компьютер и присвоения ему определенного имени  необходимо выполнить команду:

    git remote add <name> <url>
Где <"url"> - ссылка на свой репозиторий на Github. 
...
![Fork](https://img.icons8.com/?size=64&id=3R9nAUpqS3iJ&format=png)

В результате получится копия исходного репозитория в локальной учетной записи, можно вносить и сохранять изменения.

### Создание запроса на слияние Pull request

После внесения желаемых изменений в локальной копии исходного репозитория отправляется запрос на разрешение применения изменений в исходный репозиторий. Запрос на изменения называется "Pull request"

На странице локального репозитория в Github после применения изменений появится запрос на создание запроса на слияние веток "Compare & Pull request".
![Pull request](https://static.codex.so/upload/redactor_images/o_6a55b69a503812298070a566ca6c1c63.jpg)

После создания запроса на слияние (путём нажатия кнопки «Create pull request»)
владелец исходного репозитория получит уведомление о предложенных изменениях со ссылкой на страницу с информацией о запросе.
Владелец исходного репозитория может просмотреть предложенные изменения, принять, отклонить или прокомментировать их.
